...
import { computeYDomainsByAxis } from "./axes/y-domain-utils";
import {
...
} from "./patterns/reference-area-config";
import { ReferenceAreaRegistrationContext } from "./patterns/reference-area-registration-context";
import {
...
const TimeSeriesChartCore = memo(function TimeSeriesChartCoreImpl({
  ...
  const resolveYDomain = useCallback(
    (sourceData: Record<string, unknown>[], dataKeys: string[]) => {
      ...
      const domainMax =
        usesDefaultOnly && yScaleDomainMax !== undefined
          ? yScaleDomainMax
          : undefined;
  ...
  const columnWidth = useMemo(() => {
    const slotCount =
      xDomain && xDomainSlotCount !== undefined
        ? xDomainSlotCount
  ...
  const yScale = getPrimaryYScale(
    yScales,
    scaleLinear({ domain: [0, 100], nice: true, range: [innerHeight, 0] })
  );

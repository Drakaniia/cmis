...
const ExpiryBars = memo(function ExpiryBars({
  urgencyMap,
}: {
  ...
  return (
    <g>
      {data.map((d, i) => {
        ...
        const value = d.count as number;
        const barHeight = bandWidth ?? 0;
        const y = bandPos;
        const x = 0;
        const barW = (yScale(value) as number) ?? 0;
        const fill =
          urgencyMap.get(String(d.label)) === null
            ? "var(--chart-3)"
            : urgencyFill[urgencyMap.get(String(d.label))!];
        const isFaded = hoveredBarIndex !== null && hoveredBarIndex !== i;
        const rx = 6;
        const barKey = `expiry-${String(d.label)}-${i}`;

        const handleClick = () => {
          const monthKey = d.monthKey as string | undefined;
          if (monthKey) {
            void navigate({ to: "/admin/inventory/expiry" });
          }
        };
      ...
      <div className="relative overflow-hidden rounded-md border bg-card">
        ...
        >
          ...
          <ChartTooltip
            rows={(point) => {
              const urgency =
                (point.urgency as ExpiryBucket["urgency"]) ?? "caution";
              return [
                {
